...
- Picking up the enhanced shotgun shows: `你已拾取超级无敌散弹枪，伤害四倍`.
- The enhanced shotgun auto-refills its chamber, tube, and reserve 12 gauge ammo.
...
- `reserve_ammo_amount`: reserve `Ammo12gauge` amount kept on enhanced shotgun users.
...
- `reserve_ammo_amount`：强化散弹枪使用者保持的 `Ammo12gauge` 备用弹药数量。
